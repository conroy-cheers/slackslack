...
pub fn render_billboard(
    ...
    let px_w = width;
    let px_h = height * 2;
    let fb = render_billboard_rgb(texture, px_w, px_h, tick);
...
mod tests {
    ...
    fn front_face_preserves_transparent_padding() {
        ...
        let px_w = 160;
        let px_h = 120;
        let fb = render_billboard_rgb(&texture, px_w, px_h, 0);
    ...
    fn back_face_reads_as_billboard_rear() {
        ...
        let px_w = 180;
        let px_h = 140;
        let front = render_billboard_rgb(&texture, px_w, px_h, 0);
        let back_tick = (std::f64::consts::PI / 0.04).round() as u64;
        let back = render_billboard_rgb(&texture, px_w, px_h, back_tick);
